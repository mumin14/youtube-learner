...
export default function SharedNotesPage() {
  ...
  const [data, setData] = useState<SharedNotesData | null>(null);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    fetch(`/api/share/notes/${token}`)
      .then((res) => {
        if (!res.ok) throw new Error("Not found");
        return res.json();
      })
      .then(setData)
      .catch(() => setError("These shared notes were not found or have been removed."));
  }, [token]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-accent/30">
      ...
      <div className="container mx-auto max-w-3xl px-6 py-12">
        <div className="mb-8 space-y-2">
          ...
          <h1 className="text-2xl font-bold">{data.ownerName}&apos;s Study Notes</h1>
          <p className="text-sm text-muted-foreground">
            {data.notes.length} study {data.notes.length === 1 ? "note" : "notes"} &middot; Shared via Socraty AI
          </p>
        </div>

        {data.notes.length === 0 ? (
          <p className="text-muted-foreground text-center py-12">No study notes to show.</p>
        ) : (
